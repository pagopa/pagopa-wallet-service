...
class LogoConfig {
    ...
    @Bean
    fun walletLogoMapping(@Value("#{\${wallet.logo_mapping}}") logoMapping: Map<String, String>): Map<String, URI> {
        val transformedLogoMapping = logoMapping.mapValues {
            URI.create(it.value)
        }
        val expectedKeys =
            (
                    WalletCardDetailsDto.BrandEnum.values().map { it.toString() }
                            +
                            WalletDetailsType.values().filter { it != WalletDetailsType.CARDS }.map { it.toString() }
